...
subroutine residual_fit_gamma(n,params,l_sum)
 integer, intent(in) :: n
 real ( kind = rk ), intent(in)  :: params(n) ! guess for  solution
 real ( kind = rk ), intent(out) :: l_sum(n) ! function evaluated for given params
 integer :: k

 ! solve for moments 3 and 4 (k=2,3)
 do k=1,n
    ! lsum is the error between the desired moments and the moments of the distribution
    l_sum(k) = gamma_func_moment(n,params,mu,k+1)/mu(k+2) - 1.
 enddo
...
! and parameters d_on_p and p (params)
!
! can either give d_on_p and p (n=2), 
! or d alone (n=1) in which case p=1
!----------------------------------------------------
real function gamma_func_moment(n,params,mu,k)
 integer, intent(in) :: n,k
 real, intent(in) :: params(n)
 real, intent(in) :: mu(2)
 real :: d_on_p,theta,p
